...
public class BookDAOImpl implements BookDAO {
    ...
    public boolean updateBook(Book book) throws DatabaseException {
        String sql = "UPDATE books SET title=?, author=?, isbn=?, genre=?, publisher=?, " + "publication_date=?, total_copies=?, available_copies=?, status=?, description=? " +  "WHERE book_id=?";
        ...
        try (Connection conn = dbConnection.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.setString(4, book.getGenre());
            pstmt.setString(5, book.getPublisher());
            pstmt.setDate(6, book.getPublicationDate() != null ?  Date.valueOf(book.getPublicationDate()) : null);
            pstmt.setInt(7, book.getTotalCopies());
            pstmt.setInt(8, book.getAvailableCopies());
            pstmt.setString(9, book.getStatus());
            pstmt.setString(10, book.getDescription());
